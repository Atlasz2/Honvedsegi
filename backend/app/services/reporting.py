...
def report_filename_base(template: str, focus_type: str | None = None) -> str:
    return {
...
def serialize_focus(item: Any, focus_type: str, focus_id: str) -> dict[str, Any]:
    if focus_type == "duty":
        return {
            "type": focus_type,
            "id": focus_id,
            "headline": f"Szolgalat: {item.type}",
            "description": "",
            "participants": [],
            "details": [
    ...
    return {
        "type": focus_type,
        "id": focus_id,
        "headline": f"Megnevezes: {item.name}",
...
def build_report_data(
    ...
    def add_section(key: str, title: str, items: list[Any], itype: str, limit: int) -> None:
        visible = items[:limit]
        sections.append(
            {
                "key": key,
...
def build_excel_report_bytes(data: dict[str, Any], template: str, focus_type: str | None) -> tuple[bytes, str]:
    try:
    ...
    if data["focus"]:
        ...
        row += 1
        ws.cell(row=row, column=1, value=focus["headline"])
        row += 2
        ...
        for det in focus["details"]:
            ...
            ws.cell(row=row, column=2, value=det["value"])
            row += 1
        ...
        if focus["participants"]:
            ws.cell(row=row, column=1, value="Resztvevok").font = Font(bold=True)
            row += 1
            for col in (1, 2):
                ws.cell(row=row, column=col).fill = dark
                ws.cell(row=row, column=col).font = white_bold
            ws.cell(row=row, column=1, value="Nev")
            ws.cell(row=row, column=2, value="Reszleg")
            row += 1
        ...
        for section in data["sections"]:
            ...
            headers = (
                ["Kezdes", "Vege", "Tipus", "Szemely", "Helyszin", "Statusz"]
                if is_duty
                else ["Kezdes", "Vege", "Megnevezes", "Helyszin", "Statusz"]
            )
    ...
    filename = f"{report_filename_base(template, focus_type)}.xlsx"
    return payload, filename


def build_docx_report_bytes(data: dict[str, Any], template: str, focus_type: str | None) -> tuple[bytes, str]:
    try:
        from io import BytesIO
    ...
    if data["focus"]:
        ...
        tbl.rows[0].cells[0].text = "Reszlet"
        tbl.rows[0].cells[1].text = "Ertek"
        for det in focus["details"]:
            r = tbl.add_row().cells
            r[0].text = str(det["label"])
            r[1].text = str(det["value"])

        if focus["description"]:
            doc.add_heading("Leiras", level=3)
            doc.add_paragraph(focus["description"])

        if focus["participants"]:
            doc.add_heading("Resztvevok", level=3)
            pt = doc.add_table(rows=1, cols=2)
            pt.style = "Table Grid"
            pt.rows[0].cells[0].text = "Nev"
            pt.rows[0].cells[1].text = "Reszleg"
            for part in focus["participants"]:
                r = pt.add_row().cells
                r[0].text = str(part["personName"])
                r[1].text = str(part["detail"])
    else:
        for section in data["sections"]:
            ...
            for item in section["items"]:
                r = tbl.add_row().cells
                r[0].text = str(item.get("startDate", ""))
                r[1].text = str(item.get("endDate", ""))
                if is_duty:
                    r[2].text = str(item.get("type", ""))
                    r[3].text = str(item.get("personName", ""))
                    r[4].text = str(item.get("location", ""))
                    r[5].text = str(item.get("status", ""))
                else:
                    r[2].text = str(item.get("name", ""))
                    r[3].text = str(item.get("location", ""))
    ...
    payload = buf.getvalue()
    buf.close()
    filename = f"{report_filename_base(template, focus_type)}.docx"
    return payload, filename


def build_pdf_report_bytes(data: dict[str, Any], template: str, focus_type: str | None) -> tuple[bytes, str]:
    try:
    ...
    def ps(name: str, font: str = body_font, size: int = 9, leading: int = 12, color=C_DARK, bold: bool = False, **kw):
    ...
    hdr_tbl = Table([[Paragraph(data["title"], sTitle)], [Paragraph(subtitle, sSub)]], colWidths=[W_pt])
    hdr_tbl.setStyle(
        TableStyle(
            [
    ...
    card_data = [
        [
            Table(
                [
                    [Paragraph(v, ps(f"cv{i}", size=20, leading=24, bold=True, color=C_ACCENT))],
    ...
    def sec_hdr(title: str, count: int | None = None):
    ...
    def data_tbl(headers: list[str], rows: list[list[str]], widths: list[float]):
    ...
    if template == "focus":
        ...
        if focus["description"]:
            ...
            story.append(sec_hdr("Leiras"))
            story.append(Spacer(1, 0.2 * cm))
            for line in focus["description"].splitlines():
                story.append(Paragraph(line or " ", sDesc))
        if focus["participants"]:
            story.append(Spacer(1, 0.4 * cm))
            story.append(sec_hdr("Resztvevok", len(focus["participants"])))
            story.append(Spacer(1, 0.2 * cm))
        ...
        for sec in data["sections"]:
            ...
            story.append(Spacer(1, 0.3 * cm))
            story.append(sec_hdr(sec["title"], sec["count"]))
            story.append(Spacer(1, 0.2 * cm))
            if not sec["items"]:
                story.append(Paragraph("Nincs talalat a megadott feltetelekre.", sEmpty))
                story.append(Spacer(1, 0.2 * cm))
                continue

            def _row(item: dict[str, Any], t: str):
                if t == "duty":
                    return [
                        item.get("startDate", ""),
                        item.get("endDate", ""),
    ...
    def add_page_number(canvas_obj, _doc_obj):
        canvas_obj.saveState()
        canvas_obj.setFont(body_font, 7)
        canvas_obj.setFillColor(colors.HexColor("#94a3b8"))
        num = f"{canvas_obj.getPageNumber()}. oldal  |  Guard Guard Duty - {data['title']}"
    ...
    return pdf_data, filename_map.get(template, "riport.pdf")
