...
import { trainings as store, personnel as pStore, logAction } from '@/lib/store';
import { Training, TrainingAssignment } from '@/lib/types';
...
export default function TrainingPage() {
  ...
  const [form, setForm] = useState({ name: '', type: TYPES[0], startDate: '', endDate: '', location: '', organizer: '', maxPersonnel: 20, description: '', status: 'Tervezett' as string, assigned: [] as TrainingAssignment[] });
  ...
  const handleSave = () => {
    if (!validate()) return;
    if (editing) {
      store.update({ ...editing, ...form } as Training);
      logAction(user!.displayName, user!.username, 'módosítva', 'Kiképzések', form.name);
    } else {
      store.add(form as any);
      logAction(user!.displayName, user!.username, 'létrehozva', 'Kiképzések', form.name);
    }
  ...
  const updateAttendance = (personId: string, att: string) => {
    if (!detail) return;
    const updated = { ...detail, assigned: detail.assigned.map(a => a.personId === personId ? { ...a, attendance: att as any } : a) };
    store.update(updated);
    setDetail(updated);
    refresh();
  };
  ...
  const openEdit = (t: Training) => { setForm({ ...t } as any); setErrors({}); setEditing(t); };
    ...
    <div>
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        <select value={filter.type} onChange={e => setFilter({ ...filter, type: e.target.value })}
          className="bg-input border border-border px-3 py-1.5 text-xs uppercase" style={{ borderRadius: '2px' }}>
          <option value="">Minden típus</option>
          {TYPES.map(t => <option key={t} value={t}>{t}</option>)}
        </select>
        {['', ...STATUSES].map(s => (
          <button key={s} onClick={() => setFilter({ ...filter, status: s })}
            className={`px-3 py-1.5 text-xs uppercase tracking-military font-mono ${filter.status === s ? 'btn-mil-primary' : 'btn-mil-secondary'}`}>
            {s || 'Összes'}
          </button>
        ))}
      </div>
              ...
              <tbody>
                {detail.assigned.map(a => (
                  <tr key={a.personId}>
                    ...
                    <td>
                      {canEdit ? (
                        <select value={a.attendance} onChange={e => updateAttendance(a.personId, e.target.value)}
                          className="bg-input border border-border px-2 py-1 text-xs" style={{ borderRadius: '2px' }}>
                    ...
                    {canEdit && <td><button onClick={() => {
                      ...
                      store.update(updated); setDetail(updated); refresh();
      ...
      {/* Create/Edit Modal */}
      <Modal open={creating || !!editing} onClose={() => { setCreating(false); setEditing(null); }} title={editing ? 'Kiképzés szerkesztése' : 'Új kiképzés'}>
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div><label className="block text-xs uppercase tracking-military text-muted-foreground mb-1">Kezdete *</label>
              <input type="date" value={form.startDate} onChange={e => setForm({ ...form, startDate: e.target.value })} className="w-full bg-input border border-border px-3 py-2 text-sm" style={{ borderRadius: '2px' }} />
            ...
            <div><label className="block text-xs uppercase tracking-military text-muted-foreground mb-1">Vége *</label>
              <input type="date" value={form.endDate} onChange={e => setForm({ ...form, endDate: e.target.value })} className="w-full bg-input border border-border px-3 py-2 text-sm" style={{ borderRadius: '2px' }} />
          ...
          <div><label className="block text-xs uppercase tracking-military text-muted-foreground mb-1">Státusz</label>
            <select value={form.status} onChange={e => setForm({ ...form, status: e.target.value })} className="w-full bg-input border border-border px-3 py-2 text-sm" style={{ borderRadius: '2px' }}>
      ...
      <ConfirmDialog open={!!deleteTarget} onClose={() => setDeleteTarget(null)} onConfirm={() => {
        if (deleteTarget) { store.remove(deleteTarget.id); logAction(user!.displayName, user!.username, 'törölve', 'Kiképzések', deleteTarget.name); toast.success('Törölve'); setDetail(null); refresh(); }
      }} />
    </div>
  );
}
