...
import { Calendar, MapPin, Plus, Search, Trash2, Users } from "lucide-react";
import { events, getErrorMessage } from "@/lib/store";
...
export default function Events() {
  ...
  const handleCreate = async () => {
    if (!validate()) return;
    try {
      await events.add({ ...form, name: form.name.trim(), location: form.location.trim(), organizer: form.organizer.trim(), description: form.description.trim() });
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        {canEdit && <button onClick={() => { setForm(emptyForm); setErrors({}); setCreating(true); }} className="btn-mil-primary flex items-center gap-2 text-xs"><Plus className="w-4 h-4" />Új esemény</button>}
      </div>
      ...
      <Modal open={creating} onClose={() => setCreating(false)} title="Új esemény">
